...
def count_even(numbers):
    ...
    first_is_even = numbers[0] % 2 == 0
    return int(first_is_even) + count_even(numbers[1:])


# Uncomment to test count_even
print("\nTest count_even:")
print(
    f"  count_even([1, 2, 3, 4, 5, 6]) = {count_even([1, 2, 3, 4, 5, 6])} (expected: 3)"
)
...
def find_strings_with(strings, target):
    ...
    matches_in_rest = find_strings_with(strings[1:], target)
    if target in strings[0]:
        return [strings[0]] + matches_in_rest

    return matches_in_rest
...
def find_infected_files(directory_path, extension=".encrypted"):
    ...
    for name in os.listdir(directory_path):
        full_path = os.path.join(directory_path, name)
        matches.extend(find_infected_files(full_path, extension))
...
if __name__ == "__main__":
    ...
    print(
        ...
        len(find_infected_files("breach_data")),
    )  # ???

    ## 5. Determine how many files were corrupted by department (Finance, HR, and Sales)

    for department in ("Finance", "HR", "Sales"):
        path = os.path.join("breach_data", department)

        if os.path.isdir(path):
            infected_count = len(find_infected_files(path))
            print(f"{department}: {infected_count} infected files")
        else:
            print(f"{department}: directory not generated")
